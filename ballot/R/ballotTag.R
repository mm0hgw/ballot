...
#'@export
ballotTag <- function(x, ballot = NULL, bTitle = NULL, bRegionTag = NULL) {
...
plot.ballotTag <- function(x, ...) {
    ...
    if (!("ylim" %in% names(arg))) {
        ...
        arg$ylim[2] <- min(arg$ylim[2], max(1.5 * c(max(dList[[1]]$y), dnorm(0))))
    }
    do.call(sp::plot, arg)
    col <- seq(len) + 1
...
ls.ballotTag <- function(pattern = ".*") {
    grep(pattern, gsub("\\.ballot$", "", ls(envir = ballotEnv, pattern = "\\.ballot$")), 
        value = TRUE)
}
...
set.bTitle <- function(x, bTitle) {
    if (is.valid.tag(x) && is.valid.tag(bTitle)) {
        ...
        if (!exists(x, envir = ballotEnv) || get(x, envir = ballotEnv) != bTitle) {
...
set.bRegionTag <- function(x, bRegionTag) {
    ...
    if (is.valid.tag(x) && inherits(bRegionTag, "regionTag")) {
        ...
        if (!exists(x, envir = ballotEnv) || get(x, envir = ballotEnv) != bRegionTag) {
...
save.ballotEnv <- function() {
    xzSave(list = ls(ballotEnv), file = ballotFile, envir = ballotEnv)
}
