...
do.sbList <- function(sbList, name) {
    ...
    arg <- list(x = 0, type = "n", xlim = ballot:::dListXlim(dList), ylim = ballot:::dListYlim(dList), 
        main = paste(name, "'10-'15"), ylab = "Density", xlab = "Fractional Turnout")
    col <- seq_along(dList) + 1
    testFile <- paste(sep = "", "test/pics/fingerprints/", name, "_sample")
    if ("pdf" == outputType) {
    ...
    lapply(seq_along(dList), function(i) {
        ...
        lines(x, dnorm(x, mean = sbPopMean(sbList[[i]]), sd = sbPopSd(sbList[[i]])), 
            lty = 2)
    ...
    leg <- paste(names(sbList), format(digits = 4, sapply(sbList, sbPopMean) * 100), 
        "%", sapply(sbList, sbSum), "/", sapply(sbList, sbSumN))
    ...
    lapply(seq_along(sbList), function(i) {
        ...
        # if(buildPackageLoaded)gitAdd(print(testFile))
        testFile <- paste(sep = "", "test/pics/fingerprints/", name, ".", year)
        if ("pdf" == outputType) 
            pdf(paste0(testFile, ".pdf"), paper = pdfPaper)
        if ("png" == outputType) 
            png(paste0(testFile, ".png"), height = pngHeight, width = pngWidth)
        tag <- ls.ballotTag(paste(sep = ".", "Scotland", year))
        spatialPlot(tag, sample = sbCalculateSample(sb, norm = FALSE), center = sbPopMean(sb), 
    ...
    arg <- list(x = 0, type = "n", xlim = ballot:::dListXlim(dList), ylim = ballot:::dListYlim(dList), 
        main = paste(name, "faction '10-'15"), ylab = "Density", xlab = "SDs from Population mean")
    col <- seq_along(dList) + 1
    testFile <- paste(sep = "", "test/pics/fingerprints/", name, "_norm")
    if ("pdf" == outputType) 
        pdf(paste0(testFile, ".pdf"), paper = pdfPaper)
    if ("png" == outputType) 
        png(paste0(testFile, ".png"), height = pngHeight, width = pngWidth)
    ...
    legend("topright", legend = names(sbList), col = col, pch = seq_along(dList), 
        lwd = 2)
    dev.off()
    # if(buildPackageLoaded)gitAdd(print(testFile))
}

sbListYes <- list(YeSNP2010 = GE2010$SNP, YeSNP2014 = SIR2014$Yes, YeSNP2015 = GE2015$SNP)

sbListAbstain <- list(Abstainers.2010 = sbAbstainers(GE2010$V), Abstainers.2014 = sbAbstainers(SIR2014$V), 
...
sbListNo <- list(Unionist.2010 = GE2010U, Unionist.2014 = SIR2014$No, Unionist.2015 = GE2015U)
sbListCon <- list(Conservative.2010 = GE2010$Conservative, Conservative.2015 = GE2015$Conservative)

sbListLab <- list(Labour.2010 = GE2010$Labour, Labour.2015 = GE2015$Labour)

sbListLD <- list(Liberal.Democrat.2010 = GE2010[["Liberal Democrat"]], Liberal.Democrat.2015 = GE2015[["Liberal Democrat"]])
