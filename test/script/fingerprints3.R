do.party <- function(party, baseDir, bList) {
    ...
    lapply(seq_along(dList), function(i) {
        ...
        if (sbSum(sb) != 0) {
            ...
            })), col = pcol, pch = i, lwd = 2)
        }
    })
    leg <- paste(names(bList), format(digits = 4, sapply(sbList, sbPopMean) * 100), 
    ...
    lapply(seq_along(sbList), function(i) {
        ...
        if (sbSum(sb) > 0) {
            ...
            y2 <- (strsplit(y1, "\\.")[[1]])
            year <- (y2[length(y2) - 1])
            testFile <- paste(sep = "", baseDir, gsub(" ", ".", name), ".", year, 
                ".csv")
    ...
    arg <- list(x = 0, type = "n", xlim = ballot:::dListXlim(dList), ylim = ballot:::dListYlim(dList), 
        main = paste(name, "'10-'15"), ylab = "Density", xlab = "SDs from Population mean")
    col <- seq_along(dList) + 1
...
do.region <- function(region, country = "UK", ballot = "Wm") {
    ...
    print(sbTags)
    bList <- lapply(sbTags, function(x) {
...
regions <- unique(lapply(strsplit(ls.ballotTag("Wm"), "\\."), function(x) {
