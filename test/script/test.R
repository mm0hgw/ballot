...
lapply(ls.ballotTag(), function(ballotTagName) {
    ...
    lapply(seq_along(sbList), function(i) {
        ...
        if (length(grep("^SF$|^Sinn", name)) > 0) 
            name <- "Sinn Fein"
        sb <- sbList[[i]]
        testFile <- paste(sep = "", "test/pics/ballot/sp/", ballotTagName, "_ballotTag_test_", 
            i, "_", gsub(" ", ".", name), testSuffix)
        title <- paste(sep = ", ", name, year)
        subTitle <- paste(sep = "/", sbSum(sb), sbSumN(sb))
        testFun(testFile)
        spatialPlot(x, sample = as.integer(format(digits = 3, sbCalculateSample(sb, 
            norm = FALSE) * 100)), main = title, sub = subTitle)
        dev.off()
        if (buildPackageLoaded) 
            gitAdd(print(testFile))
        # testFile <- paste(sep='', 'test/pics/freqPlot_',ballotTagName,'_',i,'_', gsub('
        # ','.',name),testSuffix ) testFun(testFile) freqPlot(x,i) dev.off()
